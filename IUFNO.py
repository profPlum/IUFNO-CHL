...
class SpectralConv4d(nn.Module):
    ...
    def forward(self, x):
        ...
        x_ft = torch.fft.rfftn(x, dim=[-4,-3,-2,-1]) # Dwyer: The "w" dimension is indeed convolved over
...
# Dwyer: GOTCHA: this flattens the spatial dimensions into a single dimension then applies non-unit CNN layers!
class U_net(nn.Module):
    ...
    def forward(self, x):
        ...
        #concat0 = torch.cat((x, out_deconv0.view(batchsize, width, 32, 33, 16, 3)), 1) # Dwyer: x=32, y=33, z=16, w=3
        concat0 = torch.cat((x, out_deconv0.view(batchsize, width, *spatial)), 1)
        out = self.output_layer(concat0.view(concat0.shape[0], concat0.shape[1], -1)).view(batchsize, width, *spatial)
...
# Dwyer: this is the main model
class IUFNO4d(BasicLightningRegressor):
    def __init__(self, in_channels, out_channels, hidden_channels, k_modes:list|int=32, k_size:int=3, n_layers=4, ndims=3, hidden_norm_groups=0, out_norm_groups=0):
        super(IUFNO4d, self).__init__()

        """
        input: the solution of the first 1 timesteps + 4 locations (u(1, x, y, z), ..., u(1, x, y, z),  x, y, z, t). It's a constant function in time, except for the last index.
        output: the solution of the next timestep
        """

        if ndims != 3: raise NotImplementedError("IUFNO4d only supports ndims=3 (3D spatial dimensions)")
        if in_channels!=out_channels: raise NotImplementedError("in_channels must equal out_channels, (they are held fixed in the W/component dimension)")

        if type(k_modes) is int:
            k_modes = [k_modes]*ndims
        assert type(k_size) is int and k_size > 0
        self.modes1, self.modes2, self.modes3 = k_modes
        self.modes4 = in_channels # 3 for u,v,w (we need to cram channels into the modes4 dimension because of their crazy component convolution)
        self.width = hidden_channels
        input_time_steps = 1
        self.fc0 = nn.Linear(input_time_steps + 4, self.width)  # 1+4 (u(1, x, y), ..., u(1, x, y), x,y,z,t)
        self.nlayer = n_layers

        self.convlayer = nn.ModuleList([SpectralConv4d(self.width, self.width, self.modes1, self.modes2, self.modes3, self.modes4) for i in range(1)])
        self.w = nn.ModuleList([nn.Conv1d(self.width, self.width, 1) for i in range(1)]) # Dwyer: this is literally the W bias from the FNO paper!
        self.u = nn.ModuleList([U_net(self.width, self.width, k_size, 0) for i in range(ndims)])
    ...
    # Accepts ONLY: [B, C, X, Y, Z]  (channels-first with components in C; single input timestep)
    def forward(self, x):
        # Normalize input to [B, X, Y, Z, W, T=1]
        assert x.ndim == 5, f'Expected input of shape [B,C,X,Y,Z], got {tuple(x.shape)}'
        x = x.permute(0, 2, 3, 4, 1)[..., None]   # -> [B,X,Y,Z,W=C,T=1]
        ...
        x = self.fc0(x) # Dwyer: apparently linear layers require the feature dimension to be last...
        x = x.permute(0, 5, 1, 2, 3, 4) # Dwyer: then it seems they move the feature dimension to the channel axis (for CNNs)
        coef = 1./self.nlayer
        ...
        for i in range(self.nlayer-1):
            ...
            x3 = self.u[0](x-x1) # Dwyer: why the subtraction?
        ...
        x = x.permute(0, 2, 3, 4, 5, 1) # Dwyer: channel dimension to last again
        x = self.fc1(x)
        x = F.gelu(x)
        x = self.fc2(x).permute(0, 4, 1, 2, 3, 5)[..., 0] # Confirmed: x.shape=[batch, X, Y, Z, W, T=1] -> [batch, W, X, Y, Z]
...
# deprecated training code (interface changed)
if __name__ == '__main__':
    ...
    # GOTCHA: using a random split here is crazy
    full_set = torch.utils.data.TensorDataset(input_set.to('meta'), output_set.to('meta'))
